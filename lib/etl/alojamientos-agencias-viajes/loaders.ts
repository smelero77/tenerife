/**
 * Database loaders with batch upserts for CKAN alojamientos y agencias de viajes tables
 */

import { supabaseAdmin } from '@/lib/supabase/admin';
import {
  BronzeAlojamientoAgenciaRecord,
  SilverAlojamientoAgenciaRecord,
} from './types';
import { BATCH_SIZE } from '../nomenclator/constants';

/**
 * Batch insert into bronze_ckan_alojamientos_agencias_viajes_raw
 */
export async function loadAlojamientosAgenciasBronze(
  records: BronzeAlojamientoAgenciaRecord[]
  ...
  for (let i = 0; i < records.length; i += BATCH_SIZE) {
    ...
    try {
      const { error } = await supabaseAdmin
        .from('bronze_ckan_alojamientos_agencias_viajes_raw')
        .insert(batch);
 ...
 */
function deduplicateBatch(batch: SilverAlojamientoAgenciaRecord[]): SilverAlojamientoAgenciaRecord[] {
  const seen = new Set<string>();
  const deduplicated: SilverAlojamientoAgenciaRecord[] = [];

  for (const record of batch) {
    // Create unique key matching the functional unique index:
    // source_resource_id, establecimiento_nombre, municipio_nombre, latitud, longitud, direccion
    ...
    const direccionStr = record.establecimiento_direccion || '';
    const municipioStr = record.municipio_nombre || '';
    
    const key = `${record.source_resource_id}|${record.establecimiento_nombre}|${municipioStr}|${latitudStr}|${longitudStr}|${direccionStr}`;
...
/**
 * Batch upsert into silver_alojamiento_agencia_viaje
 * Validates INE codes against dim_municipio before inserting
 */
export async function loadAlojamientosAgenciasSilver(
  records: SilverAlojamientoAgenciaRecord[]
  ...
  const validRecords: SilverAlojamientoAgenciaRecord[] = [];
  ...
  for (let i = 0; i < validRecords.length; i += BATCH_SIZE) {
    ...
    try {
      ...
      const { error } = await supabaseAdmin.rpc('upsert_alojamiento_agencia_viaje_batch', {
...
/**
 * Refresh aggregated facts in fact_alojamiento_agencia_municipio_agg
 * Uses stored procedure that aggregates directly from silver layer
 */
export async function refreshAlojamientosAgenciasFacts(): Promise<{
  refreshed: number;
  errors: number;
}> {
  try {
    ...
    const { error: refreshError } = await supabaseAdmin.rpc(
      'refresh_fact_alojamiento_agencia_municipio_agg'
    );
    ...
    const { count } = await supabaseAdmin
      .from('fact_alojamiento_agencia_municipio_agg')
      .select('*', { count: 'exact', head: true });

    return { refreshed: count || 0, errors: 0 };
  } catch (error) {
    console.error('Error refreshing alojamientos agencias facts:', error);
