/**
 * Main orchestrator for CKAN Alojamientos y Agencias de Viajes ETL pipeline
 */

import { supabaseAdmin } from '@/lib/supabase/admin';
import { fetchAllRecords } from '@/lib/ckan/client';
import { transformAlojamientoAgenciaRecord } from './transform';
import {
  loadAlojamientosAgenciasBronze,
  loadAlojamientosAgenciasSilver,
  refreshAlojamientosAgenciasFacts,
} from './loaders';
import {
  RawCkanAlojamientoAgenciaRecord,
  BronzeAlojamientoAgenciaRecord,
  SilverAlojamientoAgenciaRecord,
} from './types';
import { BATCH_SIZE } from '../nomenclator/constants';

const CKAN_DATASET_ID = 'd9cffe8d-3ff5-43b4-864a-b246c8b98cb7';
const CKAN_RESOURCE_ID = '539bc405-514f-4966-ad7f-2437eaaefe73';
...
async function createEtlRun(): Promise<string> {
  const { data, error } = await supabaseAdmin
    .from('etl_runs')
    .insert({
      pipeline_name: 'ckan_alojamientos_agencias_viajes',
      status: 'running',
      metadata: {
 ...
 */
export async function runAlojamientosAgenciasViajes(): Promise<{
  success: boolean;
  runId: string;
  ...
  try {
    ...
    let silverRecords: SilverAlojamientoAgenciaRecord[] = [];
    ...
    try {
      const bronzeRecords: BronzeAlojamientoAgenciaRecord[] = rawRecords.map(
        (record) => ({
      ...
      const bronzeResult = await loadAlojamientosAgenciasBronze(bronzeRecords);
    ...
    try {
      ...
      for (const rawRecord of rawRecords) {
        const transformed = transformAlojamientoAgenciaRecord(
          rawRecord as RawCkanAlojamientoAgenciaRecord,
          CKAN_DATASET_ID,
          CKAN_RESOURCE_ID
        );
    ...
    try {
      ...
      const silverResult = await loadAlojamientosAgenciasSilver(silverRecords);
    ...
    try {
      const factsResult = await refreshAlojamientosAgenciasFacts();
