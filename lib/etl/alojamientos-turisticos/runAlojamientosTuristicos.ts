/**
 * Main orchestrator for CKAN Alojamientos Turísticos ETL pipeline
 */

import { supabaseAdmin } from '@/lib/supabase/admin';
import { fetchAllRecords } from '@/lib/ckan/client';
import { transformAlojamientoRecord, mapMunicipioToIneCode, buildMunicipiosMap } from './transform';
import {
  loadAlojamientosBronze,
  loadAlojamientosSilver,
  refreshAlojamientosFacts,
} from './loaders';
import {
  RawCkanAlojamientoRecord,
  BronzeAlojamientoRecord,
  SilverAlojamientoRecord,
} from './types';
import { BATCH_SIZE } from '../nomenclator/constants';

const CKAN_DATASET_ID = '253d5bb2-c6ec-4b9c-a02c-397ba128ad59';
const CKAN_RESOURCE_ID = '4c045ca7-e1be-48cf-b3e9-dbd9dce58212';
...
async function createEtlRun(): Promise<string> {
  const { data, error } = await supabaseAdmin
    .from('etl_runs')
    .insert({
      pipeline_name: 'ckan_alojamientos_turisticos',
      status: 'running',
      metadata: {
 ...
 */
export async function runAlojamientosTuristicos(): Promise<{
  success: boolean;
  runId: string;
  ...
  try {
    ...
    let silverRecords: SilverAlojamientoRecord[] = [];
    ...
    try {
      const bronzeRecords: BronzeAlojamientoRecord[] = rawRecords.map(
        (record) => ({
      ...
      const bronzeResult = await loadAlojamientosBronze(bronzeRecords);
    ...
    try {
      ...
      for (const rawRecord of rawRecords) {
        const transformed = transformAlojamientoRecord(
          rawRecord as RawCkanAlojamientoRecord,
          CKAN_DATASET_ID,
          CKAN_RESOURCE_ID
        );
    ...
    try {
      ...
      const silverResult = await loadAlojamientosSilver(silverRecords);
    ...
    try {
      const factsResult = await refreshAlojamientosFacts();
