/**
 * Main orchestrator for CKAN Asociaciones Ciudadanas ETL pipeline
 */

import { supabaseAdmin } from '@/lib/supabase/admin';
import { fetchAllRecords } from '@/lib/ckan/client';
import { transformAsociacionRecord } from './transform';
import {
  loadAsociacionesBronze,
  loadAsociacionesSilver,
  refreshAsociacionesFacts,
} from './loaders';
import {
  RawCkanAsociacionRecord,
  BronzeAsociacionRecord,
  SilverAsociacionRecord,
} from './types';
import { BATCH_SIZE } from '../nomenclator/constants';

const CKAN_DATASET_ID = '07d27432-754c-4d49-8fa2-5ab97a734f3b';
const CKAN_RESOURCE_ID = '10553c44-8136-4fde-b3cc-7d2428308ca3';
...
async function createEtlRun(): Promise<string> {
  const { data, error } = await supabaseAdmin
    .from('etl_runs')
    .insert({
      pipeline_name: 'ckan_asociaciones_ciudadanas',
      status: 'running',
      metadata: {
 ...
 */
export async function runAsociacionesCiudadanas(): Promise<{
  success: boolean;
  runId: string;
  ...
  try {
    ...
    let silverRecords: SilverAsociacionRecord[] = [];
    ...
    try {
      const bronzeRecords: BronzeAsociacionRecord[] = rawRecords.map(
        (record) => ({
      ...
      const bronzeResult = await loadAsociacionesBronze(bronzeRecords);
    ...
    try {
      ...
      for (const rawRecord of rawRecords) {
        const transformed = transformAsociacionRecord(
          rawRecord as RawCkanAsociacionRecord,
          CKAN_DATASET_ID,
          CKAN_RESOURCE_ID
        );
    ...
    try {
      ...
      const silverResult = await loadAsociacionesSilver(silverRecords);
    ...
    try {
      const factsResult = await refreshAsociacionesFacts();
