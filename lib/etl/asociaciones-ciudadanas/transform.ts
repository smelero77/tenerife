...
import { RawCkanAsociacionRecord, SilverAsociacionRecord } from './types';
...
function normalizePhone(value: unknown): string | null {
  ...
  // Handle numeric values (e.g., 601427333.0)
  const str = typeof value === 'number' 
    ? Math.floor(value).toString() 
    : String(value).trim().split('.')[0]; // Remove decimal part
...
/**
 * Extract activity type and scope from actividad_tipo
 * Format: "asociacion <scope>" (e.g., "asociacion servicios sociales")
 */
function extractActivityAndScope(actividadTipo: string | null): {
  actividad: string | null;
  ambito: string | null;
} {
  if (!actividadTipo) {
    return { actividad: null, ambito: null };
  }
  
  const normalized = actividadTipo.trim().toLowerCase();
  
  // Extract scope (everything after "asociacion ")
  if (normalized.startsWith('asociacion ')) {
    const scope = normalized.substring('asociacion '.length).trim();
    return {
 ...
 */
export function transformAsociacionRecord(
  rawRecord: RawCkanAsociacionRecord,
  sourceDatasetId: string,
  sourceResourceId: string
): SilverAsociacionRecord | null {
  // Required field
  ...
  // Optional fields
  const siglas = normalizeString(rawRecord.nombre); // Could extract siglas from nombre if needed
  const cif = null; // Not in CSV
  ...
  return {
    ...
    asociacion_siglas: siglas, // Could be extracted from nombre if needed
    asociacion_cif: cif,
    asociacion_telefono: telefono,
    asociacion_email: email,
    asociacion_web: web,
    asociacion_direccion: direccion,
    asociacion_codigo_postal: codigoPostal,
    municipio_nombre: municipioNombre,
    ine_code: ineCode, // Will be validated against dim_municipio in loaders
    latitud,
    longitud,
    asociacion_actividad: actividad,
    asociacion_ambito: ambito,
  };
}
