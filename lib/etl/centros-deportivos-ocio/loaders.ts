/**
 * Database loaders with batch upserts for CKAN centros deportivos ocio tables
 */
...
/**
 * Batch insert into bronze_ckan_centros_deportivos_ocio_raw
 */
export async function loadCentrosBronze(
  records: BronzeCentroRecord[]
  ...
  for (let i = 0; i < records.length; i += BATCH_SIZE) {
    ...
    try {
      const { error } = await supabaseAdmin
        .from('bronze_ckan_centros_deportivos_ocio_raw')
        .insert(batch);
 ...
 */
function deduplicateBatch(batch: SilverCentroRecord[]): SilverCentroRecord[] {
  const seen = new Set<string>();
  const deduplicated: SilverCentroRecord[] = [];

  for (const record of batch) {
    // Create unique key matching the functional unique index:
    // source_resource_id, centro_nombre, municipio_nombre, latitud, longitud, direccion
    ...
    const direccionStr = record.centro_direccion || '';
    const municipioStr = record.municipio_nombre || '';
    
    const key = `${record.source_resource_id}|${record.centro_nombre}|${municipioStr}|${latitudStr}|${longitudStr}|${direccionStr}`;
...
/**
 * Batch upsert into silver_centro_deportivo_ocio
 * Validates INE codes against dim_municipio before inserting
 */
export async function loadCentrosSilver(
  records: SilverCentroRecord[]
  ...
  const validRecords: SilverCentroRecord[] = [];
  ...
  for (let i = 0; i < validRecords.length; i += BATCH_SIZE) {
    ...
    try {
      ...
      const { error } = await supabaseAdmin.rpc('upsert_centro_deportivo_ocio_batch', {
...
/**
 * Refresh aggregated facts in fact_centro_deportivo_ocio_municipio_agg
 * Uses stored procedure that aggregates directly from silver layer
 */
export async function refreshCentrosFacts(): Promise<{
  refreshed: number;
  errors: number;
}> {
  try {
    ...
    const { error: refreshError } = await supabaseAdmin.rpc(
      'refresh_fact_centro_deportivo_ocio_municipio_agg'
    );
    ...
    const { count } = await supabaseAdmin
      .from('fact_centro_deportivo_ocio_municipio_agg')
      .select('*', { count: 'exact', head: true });

    return { refreshed: count || 0, errors: 0 };
  } catch (error) {
    console.error('Error refreshing centros facts:', error);
