/**
 * Main orchestrator for CKAN Centros Deportivos y Ocio ETL pipeline
 */

import { supabaseAdmin } from '@/lib/supabase/admin';
import { fetchAllRecords } from '@/lib/ckan/client';
import { transformCentroRecord } from './transform';
import {
  loadCentrosBronze,
  loadCentrosSilver,
  refreshCentrosFacts,
...
const CKAN_DATASET_ID = '9778eb26-533c-438a-8528-c6302c359d42';
const CKAN_RESOURCE_ID = 'ae03c796-ca25-42fe-8a7a-d54dc015a437';
...
async function createEtlRun(): Promise<string> {
  const { data, error } = await supabaseAdmin
    .from('etl_runs')
    .insert({
      pipeline_name: 'ckan_centros_deportivos_ocio',
      status: 'running',
      metadata: {
 ...
 */
export async function runCentrosDeportivosOcio(): Promise<{
  success: boolean;
  runId: string;
  ...
  try {
    ...
    let silverRecords: SilverCentroRecord[] = [];
    ...
    try {
      const bronzeRecords: BronzeCentroRecord[] = rawRecords.map(
        (record) => ({
      ...
      const bronzeResult = await loadCentrosBronze(bronzeRecords);
    ...
    try {
      ...
      for (const rawRecord of rawRecords) {
        const transformed = transformCentroRecord(
          rawRecord as RawCkanCentroRecord,
          CKAN_DATASET_ID,
          CKAN_RESOURCE_ID
        );
    ...
    try {
      ...
      const silverResult = await loadCentrosSilver(silverRecords);
    ...
    try {
      const factsResult = await refreshCentrosFacts();
