/**
 * Database loaders with batch upserts for CKAN centros educativos culturales tables
 */

import { supabaseAdmin } from '@/lib/supabase/admin';
import {
  BronzeCentroEducativoCulturalRecord,
  SilverCentroEducativoCulturalRecord,
} from './types';
import { BATCH_SIZE } from '../nomenclator/constants';

/**
 * Batch insert into bronze_ckan_centros_educativos_culturales_raw
 */
export async function loadCentrosEducativosCulturalesBronze(
  records: BronzeCentroEducativoCulturalRecord[]
  ...
  for (let i = 0; i < records.length; i += BATCH_SIZE) {
    ...
    try {
      const { error } = await supabaseAdmin
        .from('bronze_ckan_centros_educativos_culturales_raw')
        .insert(batch);
 ...
 */
function deduplicateBatch(batch: SilverCentroEducativoCulturalRecord[]): SilverCentroEducativoCulturalRecord[] {
  const seen = new Set<string>();
  const deduplicated: SilverCentroEducativoCulturalRecord[] = [];
  const duplicates: Array<{ key: string; record: SilverCentroEducativoCulturalRecord }> = [];
  ...
  for (const record of batch) {
    ...
    const key = `${record.source_resource_id}|${record.centro_nombre}|${ineCode}|${lat}|${lon}`;
  ...
  if (duplicates.length > 0) {
    ...
    for (const dup of duplicates) {
      console.log(`  - ${dup.record.centro_nombre} (${dup.record.municipio_nombre})`);
      console.log(`    INE Code: ${dup.record.ine_code || 'N/A'}`);
      console.log(`    Dirección: ${dup.record.centro_direccion || 'N/A'}`);
...
/**
 * Batch upsert into silver_centro_educativo_cultural
 * Validates INE codes against dim_municipio before inserting
 */
export async function loadCentrosEducativosCulturalesSilver(
  records: SilverCentroEducativoCulturalRecord[]
  ...
  const validRecords: SilverCentroEducativoCulturalRecord[] = [];
  ...
  for (let i = 0; i < validRecords.length; i += BATCH_SIZE) {
    ...
    try {
      ...
      const { error } = await supabaseAdmin.rpc('upsert_centro_educativo_cultural_batch', {
...
/**
 * Refresh aggregated facts in fact_centro_educativo_cultural_municipio_agg
 * Uses stored procedure that aggregates directly from silver layer
 */
export async function refreshCentrosEducativosCulturalesFacts(): Promise<{
  refreshed: number;
  errors: number;
}> {
  try {
    ...
    const { error: refreshError } = await supabaseAdmin.rpc(
      'refresh_fact_centro_educativo_cultural_municipio_agg'
    );
    ...
    const { count } = await supabaseAdmin
      .from('fact_centro_educativo_cultural_municipio_agg')
      .select('*', { count: 'exact', head: true });

    return { refreshed: count || 0, errors: 0 };
  } catch (error) {
    console.error('Error refreshing centros educativos culturales facts:', error);
