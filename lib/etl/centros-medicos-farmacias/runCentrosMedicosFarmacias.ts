/**
 * Main orchestrator for CKAN Centros Médicos y Farmacias ETL pipeline
 */

import { supabaseAdmin } from '@/lib/supabase/admin';
import { fetchAllRecords } from '@/lib/ckan/client';
import { transformCentroMedicoFarmaciaRecord } from './transform';
import {
  loadCentrosMedicosFarmaciasBronze,
  loadCentrosMedicosFarmaciasSilver,
  refreshCentrosMedicosFarmaciasFacts,
} from './loaders';
import {
  RawCkanCentroMedicoFarmaciaRecord,
  BronzeCentroMedicoFarmaciaRecord,
  SilverCentroMedicoFarmaciaRecord,
} from './types';
import { BATCH_SIZE } from '../nomenclator/constants';

const CKAN_DATASET_ID = '7d98949a-1e2f-4bdc-9280-83b81da0be35';
const CKAN_RESOURCE_ID = 'e5b336b7-c4ec-4969-adcb-66a7351b7061';
...
async function createEtlRun(): Promise<string> {
  const { data, error } = await supabaseAdmin
    .from('etl_runs')
    .insert({
      pipeline_name: 'ckan_centros_medicos_farmacias',
      status: 'running',
      metadata: {
 ...
 */
export async function runCentrosMedicosFarmacias(): Promise<{
  success: boolean;
  runId: string;
  ...
  try {
    ...
    let silverRecords: SilverCentroMedicoFarmaciaRecord[] = [];
    ...
    try {
      const bronzeRecords: BronzeCentroMedicoFarmaciaRecord[] = rawRecords.map(
        (record) => ({
      ...
      const bronzeResult = await loadCentrosMedicosFarmaciasBronze(bronzeRecords);
    ...
    try {
      ...
      for (const rawRecord of rawRecords) {
        const transformed = transformCentroMedicoFarmaciaRecord(
          rawRecord as RawCkanCentroMedicoFarmaciaRecord,
          CKAN_DATASET_ID,
          CKAN_RESOURCE_ID
        );
    ...
    try {
      ...
      const silverResult = await loadCentrosMedicosFarmaciasSilver(silverRecords);
    ...
    try {
      const factsResult = await refreshCentrosMedicosFarmaciasFacts();
