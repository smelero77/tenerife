...
import { RawCkanComercioAgriculturaRecord, SilverComercioAgriculturaRecord } from './types';
...
function normalizePostalCode(value: unknown): string | null {
  ...
  // Handle numeric values (e.g., 38639.0)
...
function normalizePhone(value: unknown): string | null {
  ...
  // Handle numeric values (e.g., 922735463.0)
  const str = typeof value === 'number' 
    ? Math.floor(value).toString() 
    : String(value).trim().split('.')[0]; // Remove decimal part
 ...
 * Format: "agricultura suministros", "agricultura cooperativas", "agricultura ganaderia piensos", etc.
 */
 ...
 */
export function transformComercioAgriculturaRecord(
  rawRecord: RawCkanComercioAgriculturaRecord,
  sourceDatasetId: string,
  sourceResourceId: string
): SilverComercioAgriculturaRecord | null {
  // Required field
