...
import { RawCkanComercioAlimentacionRecord, SilverComercioAlimentacionRecord } from './types';
...
function normalizePostalCode(value: unknown): string | null {
  ...
  // Handle numeric values (e.g., 38360.0)
...
function normalizePhone(value: unknown): string | null {
  ...
  // Handle numeric values (e.g., 609256899.0)
  const str = typeof value === 'number' 
    ? Math.floor(value).toString() 
    : String(value).trim().split('.')[0]; // Remove decimal part
 ...
 * Format: "alimentacion panaderia pasteleria", "alimentacion congelados", "alimentacion", etc.
 */
 ...
 */
export function transformComercioAlimentacionRecord(
  rawRecord: RawCkanComercioAlimentacionRecord,
  sourceDatasetId: string,
  sourceResourceId: string
): SilverComercioAlimentacionRecord | null {
  // Required field
