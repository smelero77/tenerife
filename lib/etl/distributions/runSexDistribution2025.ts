/**
 * Main orchestrator for Sex Distribution 2025 ETL pipeline
 */

import { supabaseAdmin } from '@/lib/supabase/admin';
import { parseSexCsvStream } from './parseSexCsv';
import { transformSexRow } from './transformSex';
import {
  loadSexBronze,
  loadSexSilver,
  loadSexFactsLocalidad,
  loadSexFactsMunicipio,
} from './loaders';
import {
  BronzeSexRecord,
  SilverSexRecord,
...
import { SEX_CSV_PATH } from './constants';
import { join } from 'path';

const CSV_FILE_PATH = join(process.cwd(), SEX_CSV_PATH);
...
async function createEtlRun(): Promise<string> {
  const { data, error } = await supabaseAdmin
    .from('etl_runs')
    .insert({
      pipeline_name: 'sex_distribution_multi_year_tenerife',
      status: 'running',
        metadata: {
          years: '2000-2025',
          scope: 'Tenerife',
...
export async function runSexDistribution2025(): Promise<{
  success: boolean;
  runId: string;
  steps: StepResult[];
  ...
  try {
    ...
    const bronzeRecords: BronzeSexRecord[] = [];
    const silverRecords: SilverSexRecord[] = [];
    let tenerifeRowCount = 0;

    try {
      console.log(`Reading Sex CSV from: ${CSV_FILE_PATH}`);
      for await (const { row: rawRow, year } of parseSexCsvStream(CSV_FILE_PATH)) {
        summary.totalRowsProcessed++;

        // Transform and filter
        const parsed = transformSexRow(rawRow, year, CSV_FILE_PATH);
        if (!parsed) {
          summary.skippedRows++;
          continue;
        }
    ...
    try {
      const bronzeResult = await loadSexBronze(bronzeRecords);
    ...
    try {
      const silverResult = await loadSexSilver(silverRecords);
    ...
    // Step 4: Build and load Sex Facts Localidad (NUC/DIS only)
    const step4Id = await createEtlStep(runId, 'load_sex_facts_localidad');
    const step4Start = Date.now();
    try {
      const sexFactsLocalidad: SexFactLocalidadRecord[] = silverRecords
      ...
      const factsResult = await loadSexFactsLocalidad(sexFactsLocalidad);
      const step4Duration = Date.now() - step4Start;
      await updateEtlStep(step4Id, 'completed');
      steps.push({
        stepName: 'load_sex_facts_localidad',
        duration: step4Duration,
        success: true,
        stats: factsResult,
      });
      summary.sexFactsLocalidadInserted = factsResult.inserted;
    } catch (error) {
      const step4Duration = Date.now() - step4Start;
      const errorMsg =
        error instanceof Error ? error.message : String(error);
    ...
    // Step 5: Build and load Sex Facts Municipio (Tipo='M' only)
    const step5Id = await createEtlStep(runId, 'load_sex_facts_municipio');
    const step5Start = Date.now();
    try {
      const sexFactsMunicipio: SexFactMunicipioRecord[] = silverRecords
        .filter((r) => r.tipo === 'M')
        .map((r) => ({
          ine_code: r.ine_code,
          year: r.year,
      ...
      const factsResult = await loadSexFactsMunicipio(sexFactsMunicipio);
      const step5Duration = Date.now() - step5Start;
      await updateEtlStep(step5Id, 'completed');
      steps.push({
        stepName: 'load_sex_facts_municipio',
        duration: step5Duration,
        success: true,
        stats: factsResult,
      });
      summary.sexFactsMunicipioInserted = factsResult.inserted;
    } catch (error) {
      const step5Duration = Date.now() - step5Start;
      const errorMsg =
        error instanceof Error ? error.message : String(error);
