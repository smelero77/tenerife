/**
 * Main orchestrator for CKAN Empresas Industriales ETL pipeline
 */

import { supabaseAdmin } from '@/lib/supabase/admin';
import { fetchAllRecords } from '@/lib/ckan/client';
import { transformEmpresaIndustrialRecord } from './transform';
import {
  loadEmpresasIndustrialesBronze,
  loadEmpresasIndustrialesSilver,
  refreshEmpresasIndustrialesFacts,
} from './loaders';
import {
  RawCkanEmpresaIndustrialRecord,
  BronzeEmpresaIndustrialRecord,
  SilverEmpresaIndustrialRecord,
} from './types';
import { BATCH_SIZE } from '../nomenclator/constants';

const CKAN_DATASET_ID = 'a4aa81b3-b1ad-4b34-b7a5-cee273ab346f';
const CKAN_RESOURCE_ID = 'ef661002-2039-4c29-9fde-8cb7d1c00209';
...
async function createEtlRun(): Promise<string> {
  const { data, error } = await supabaseAdmin
    .from('etl_runs')
    .insert({
      pipeline_name: 'ckan_empresas_industriales',
      status: 'running',
      metadata: {
 ...
 */
export async function runEmpresasIndustriales(): Promise<{
  success: boolean;
  runId: string;
  ...
  try {
    ...
    let silverRecords: SilverEmpresaIndustrialRecord[] = [];
    ...
    try {
      const bronzeRecords: BronzeEmpresaIndustrialRecord[] = rawRecords.map(
        (record) => ({
      ...
      const bronzeResult = await loadEmpresasIndustrialesBronze(bronzeRecords);
    ...
    try {
      ...
      for (const rawRecord of rawRecords) {
        const transformed = transformEmpresaIndustrialRecord(
          rawRecord as RawCkanEmpresaIndustrialRecord,
          CKAN_DATASET_ID,
          CKAN_RESOURCE_ID
        );
    ...
    try {
      ...
      const silverResult = await loadEmpresasIndustrialesSilver(silverRecords);
    ...
    try {
      const factsResult = await refreshEmpresasIndustrialesFacts();
