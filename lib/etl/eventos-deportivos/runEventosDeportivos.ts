/**
 * Main orchestrator for CKAN Eventos Deportivos ETL pipeline
 * Filters: Only 2026 events, with valid municipio_nombre
 */

import { supabaseAdmin } from '@/lib/supabase/admin';
import { fetchAllRecords } from '@/lib/ckan/client';
import { transformEventoRecord, mapMunicipioToIneCode, buildMunicipiosMap } from './transform';
import {
  loadEventosBronze,
  loadEventosSilver,
  refreshEventosFacts,
...
const CKAN_DATASET_ID = '6cde1436-8d6f-4cf6-b5a5-c32c8a738478';
const CKAN_RESOURCE_ID = 'eadb729d-409d-49ed-9ffd-b68857b7afc2';
...
async function createEtlRun(): Promise<string> {
  const { data, error } = await supabaseAdmin
    .from('etl_runs')
    .insert({
      pipeline_name: 'ckan_eventos_deportivos',
      status: 'running',
      metadata: {
 ...
 */
export async function runEventosDeportivos(): Promise<{
  success: boolean;
  runId: string;
  ...
  const summary: Record<string, number> = {
    ...
    silverTransformed: 0,
    silverSkipped: 0,
    silverSkippedNoMunicipio: 0,
    silverSkippedNot2026: 0,
    silverUpserted: 0,
    silverErrors: 0,
    factsRefreshed: 0,
    factsErrors: 0,
  ...
  try {
    ...
    let silverRecords: SilverEventoRecord[] = [];
    ...
    try {
      const bronzeRecords: BronzeEventoRecord[] = rawRecords.map(
        (record) => ({
      ...
      const bronzeResult = await loadEventosBronze(bronzeRecords);
    ...
    try {
      ...
      for (const rawRecord of rawRecords) {
        const transformed = transformEventoRecord(
          rawRecord as RawCkanEventoRecord,
          CKAN_DATASET_ID,
          CKAN_RESOURCE_ID
        );
        ...
        if (!transformed) {
          ...
          const municipioNombre = String(rawRecord.municipio_nombre || '').trim();
          const fechaInicio = String(rawRecord.evento_fecha_inicio || '').trim();
          
          if (!municipioNombre || municipioNombre === '') {
            summary.silverSkippedNoMunicipio++;
          } else if (fechaInicio && !fechaInicio.startsWith('2026-')) {
            summary.silverSkippedNot2026++;
          }
          continue;
        }
      ...
      if (summary.silverSkippedNoMunicipio > 0) {
        console.log(`  - Skipped ${summary.silverSkippedNoMunicipio} records without municipio_nombre`);
      }
      if (summary.silverSkippedNot2026 > 0) {
        console.log(`  - Skipped ${summary.silverSkippedNot2026} records not in 2026`);
      }
      ...
      steps.push({
        ...
        stats: {
          ...
          skippedNoMunicipio: summary.silverSkippedNoMunicipio,
          skippedNot2026: summary.silverSkippedNot2026,
        },
      });
    } catch (error) {
      const errorMessage =
    ...
    try {
      ...
      const silverResult = await loadEventosSilver(silverRecords);
    ...
    try {
      const factsResult = await refreshEventosFacts();
