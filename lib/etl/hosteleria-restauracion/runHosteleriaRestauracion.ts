/**
 * Main orchestrator for CKAN Hostelería y Restauración ETL pipeline
 */

import { supabaseAdmin } from '@/lib/supabase/admin';
import { fetchAllRecords } from '@/lib/ckan/client';
import { transformEstablecimientoRecord, mapMunicipioToIneCode, buildMunicipiosMap } from './transform';
import {
  loadEstablecimientosBronze,
  loadEstablecimientosSilver,
  refreshEstablecimientosFacts,
} from './loaders';
import {
  RawCkanEstablecimientoRecord,
  BronzeEstablecimientoRecord,
  SilverEstablecimientoRecord,
} from './types';
import { BATCH_SIZE } from '../nomenclator/constants';

const CKAN_DATASET_ID = '4912b6d8-89f3-48d7-9dd1-bdefb18dbb17';
const CKAN_RESOURCE_ID = 'a7a7d607-0720-418e-98bb-c4e365d66e4b';
...
async function createEtlRun(): Promise<string> {
  const { data, error } = await supabaseAdmin
    .from('etl_runs')
    .insert({
      pipeline_name: 'ckan_hosteleria_restauracion',
      status: 'running',
      metadata: {
 ...
 */
export async function runHosteleriaRestauracion(): Promise<{
  success: boolean;
  runId: string;
  ...
  try {
    ...
    let silverRecords: SilverEstablecimientoRecord[] = [];
    ...
    try {
      const bronzeRecords: BronzeEstablecimientoRecord[] = rawRecords.map(
        (record) => ({
      ...
      const bronzeResult = await loadEstablecimientosBronze(bronzeRecords);
    ...
    try {
      ...
      for (const rawRecord of rawRecords) {
        const transformed = transformEstablecimientoRecord(
          rawRecord as RawCkanEstablecimientoRecord,
          CKAN_DATASET_ID,
          CKAN_RESOURCE_ID
        );
    ...
    try {
      ...
      const silverResult = await loadEstablecimientosSilver(silverRecords);
    ...
    try {
      const factsResult = await refreshEstablecimientosFacts();
