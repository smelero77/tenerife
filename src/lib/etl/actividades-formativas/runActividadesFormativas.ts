/**
 * Main orchestrator for CKAN Actividades Formativas ETL pipeline
 */

import { supabaseAdmin } from '@/lib/supabase/admin';
import { fetchAllRecords } from '@/lib/ckan/client';
import { transformActividadRecord, mapMunicipioToIneCode, buildMunicipiosMap } from './transform';
import {
  loadActividadesBronze,
  loadActividadesSilver,
  refreshActividadesFacts,
} from './loaders';
import {
  RawCkanActividadRecord,
  BronzeActividadRecord,
  SilverActividadRecord,
} from './types';
import { BATCH_SIZE } from '../nomenclator/constants';

const CKAN_DATASET_ID = '70d1454e-c560-4af0-9729-b892665d4953';
const CKAN_RESOURCE_ID = 'a55f5ad5-22f7-4ce4-8c03-81a513bf82d7';
...
async function createEtlRun(): Promise<string> {
  const { data, error } = await supabaseAdmin
    .from('etl_runs')
    .insert({
      pipeline_name: 'ckan_actividades_formativas_agrocabildo',
      status: 'running',
      metadata: {
 ...
 */
export async function runActividadesFormativas(): Promise<{
  success: boolean;
  runId: string;
  ...
  try {
    ...
    let silverRecords: SilverActividadRecord[] = [];
    ...
    try {
      const bronzeRecords: BronzeActividadRecord[] = rawRecords.map(
        (record) => ({
      ...
      const bronzeResult = await loadActividadesBronze(bronzeRecords);
    ...
    try {
      ...
      for (const rawRecord of rawRecords) {
        const transformed = transformActividadRecord(
          rawRecord as RawCkanActividadRecord,
          CKAN_DATASET_ID,
          CKAN_RESOURCE_ID
        );
    ...
    try {
      ...
      const silverResult = await loadActividadesSilver(silverRecords);
    ...
    try {
      const factsResult = await refreshActividadesFacts();
