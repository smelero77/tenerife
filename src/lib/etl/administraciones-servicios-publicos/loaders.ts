/**
 * Database loaders with batch upserts for CKAN administraciones servicios publicos tables
 */

import { supabaseAdmin } from '@/lib/supabase/admin';
import {
  BronzeAdministracionServicioPublicoRecord,
  SilverAdministracionServicioPublicoRecord,
} from './types';
import { BATCH_SIZE } from '../nomenclator/constants';

/**
 * Batch insert into bronze_ckan_administraciones_servicios_publicos_raw
 */
export async function loadAdministracionesServiciosPublicosBronze(
  records: BronzeAdministracionServicioPublicoRecord[]
  ...
  for (let i = 0; i < records.length; i += BATCH_SIZE) {
    ...
    try {
      const { error } = await supabaseAdmin
        .from('bronze_ckan_administraciones_servicios_publicos_raw')
        .insert(batch);
 ...
 * Uses: source_resource_id, administracion_nombre, ine_code, latitud, longitud
 */
function deduplicateBatch(batch: SilverAdministracionServicioPublicoRecord[]): SilverAdministracionServicioPublicoRecord[] {
  const seen = new Set<string>();
  const deduplicated: SilverAdministracionServicioPublicoRecord[] = [];
  const duplicates: Array<{ key: string; record: SilverAdministracionServicioPublicoRecord }> = [];
  ...
  for (const record of batch) {
    ...
    const key = `${record.source_resource_id}|${record.administracion_nombre}|${ineCode}|${lat}|${lon}`;
  ...
  if (duplicates.length > 0) {
    ...
    for (const dup of duplicates) {
      console.log(`  - ${dup.record.administracion_nombre} (${dup.record.municipio_nombre})`);
      console.log(`    INE Code: ${dup.record.ine_code || 'N/A'}`);
      console.log(`    Dirección: ${dup.record.administracion_direccion || 'N/A'}`);
...
/**
 * Batch upsert into silver_administracion_servicio_publico
 * Validates INE codes against dim_municipio before inserting
 */
export async function loadAdministracionesServiciosPublicosSilver(
  records: SilverAdministracionServicioPublicoRecord[]
  ...
  const validRecords: SilverAdministracionServicioPublicoRecord[] = [];
  ...
  for (let i = 0; i < validRecords.length; i += BATCH_SIZE) {
    ...
    try {
      ...
      const { error } = await supabaseAdmin.rpc('upsert_administracion_servicio_publico_batch', {
...
/**
 * Refresh aggregated facts in fact_administracion_servicio_publico_municipio_agg
 * Uses stored procedure that aggregates directly from silver layer
 */
export async function refreshAdministracionesServiciosPublicosFacts(): Promise<{
  refreshed: number;
  errors: number;
}> {
  try {
    ...
    const { error: refreshError } = await supabaseAdmin.rpc(
      'refresh_fact_administracion_servicio_publico_municipio_agg'
    );
    ...
    const { count } = await supabaseAdmin
      .from('fact_administracion_servicio_publico_municipio_agg')
      .select('*', { count: 'exact', head: true });

    return { refreshed: count || 0, errors: 0 };
  } catch (error) {
    console.error('Error refreshing administraciones servicios publicos facts:', error);
