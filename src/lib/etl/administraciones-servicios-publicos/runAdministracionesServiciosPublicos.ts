/**
 * Main orchestrator for CKAN Administraciones y Servicios Públicos ETL pipeline
 */
...
import { transformAdministracionServicioPublicoRecord } from './transform';
import {
  loadAdministracionesServiciosPublicosBronze,
  loadAdministracionesServiciosPublicosSilver,
  refreshAdministracionesServiciosPublicosFacts,
} from './loaders';
import {
  RawCkanAdministracionServicioPublicoRecord,
...
const CKAN_DATASET_ID = '5277a556-81f2-46e5-8a22-8024bfddd387';
const CKAN_RESOURCE_ID = '897f0d4e-6e93-4281-89b0-f63cbee5f330';
...
async function createEtlRun(): Promise<string> {
  const { data, error } = await supabaseAdmin
    .from('etl_runs')
    .insert({
      pipeline_name: 'ckan_administraciones_servicios_publicos',
      status: 'running',
      metadata: {
 ...
 */
export async function runAdministracionesServiciosPublicos(): Promise<{
  success: boolean;
  runId: string;
  ...
  try {
    ...
    let silverRecords: SilverAdministracionServicioPublicoRecord[] = [];
    ...
    try {
      const bronzeRecords: BronzeAdministracionServicioPublicoRecord[] = rawRecords.map(
        (record) => ({
      ...
      const bronzeResult = await loadAdministracionesServiciosPublicosBronze(bronzeRecords);
    ...
    try {
      ...
      for (const rawRecord of rawRecords) {
        const transformed = transformAdministracionServicioPublicoRecord(
          rawRecord as RawCkanAdministracionServicioPublicoRecord,
          CKAN_DATASET_ID,
          CKAN_RESOURCE_ID
        );
    ...
    try {
      ...
      const silverResult = await loadAdministracionesServiciosPublicosSilver(silverRecords);
    ...
    try {
      const factsResult = await refreshAdministracionesServiciosPublicosFacts();
