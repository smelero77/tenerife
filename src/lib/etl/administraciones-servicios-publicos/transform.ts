...
import { RawCkanAdministracionServicioPublicoRecord, SilverAdministracionServicioPublicoRecord } from './types';
...
function normalizePostalCode(value: unknown): string | null {
  ...
  // Handle numeric values (e.g., 38300.0)
...
function normalizePhone(value: unknown): string | null {
  ...
  // Handle numeric values (e.g., 922325523.0)
  const str = typeof value === 'number' 
    ? Math.floor(value).toString() 
    : String(value).trim().split('.')[0]; // Remove decimal part
  
  // Skip if value is 0.0 (which becomes "0")
  if (str === '0') {
    return null;
  }
 ...
 * Format: "administracion publica servicios tributarios", "servicios empleo", etc.
 */
 ...
 */
export function transformAdministracionServicioPublicoRecord(
  rawRecord: RawCkanAdministracionServicioPublicoRecord,
  sourceDatasetId: string,
  sourceResourceId: string
): SilverAdministracionServicioPublicoRecord | null {
  // Required field
  ...
  return {
    ...
    administracion_nombre: nombre,
    administracion_tipo: tipo,
    administracion_telefono: telefono,
    administracion_email: email,
    administracion_web: web,
    administracion_direccion: direccion,
    administracion_codigo_postal: codigoPostal,
    municipio_nombre: municipioNombre,
    ine_code: ineCode, // Will be validated against dim_municipio in loaders
    latitud,
    longitud,
    administracion_actividad: actividad,
  };
}
