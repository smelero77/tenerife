...
import { RawCkanAlojamientoAgenciaRecord, SilverAlojamientoAgenciaRecord } from './types';
...
function normalizePostalCode(value: unknown): string | null {
  ...
  // Handle numeric values (e.g., 38480.0)
...
function normalizePhone(value: unknown): string | null {
  ...
  // Handle numeric values (e.g., 922129006.0)
  const str = typeof value === 'number' 
    ? Math.floor(value).toString() 
    : String(value).trim().split('.')[0]; // Remove decimal part
 ...
 * Format: "vivienda vacacional", "alojamiento extrahotelero", "informador turistico", etc.
 */
 ...
 */
export function transformAlojamientoAgenciaRecord(
  rawRecord: RawCkanAlojamientoAgenciaRecord,
  sourceDatasetId: string,
  sourceResourceId: string
): SilverAlojamientoAgenciaRecord | null {
  // Required field
