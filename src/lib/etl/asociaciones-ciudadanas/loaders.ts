/**
 * Database loaders with batch upserts for CKAN asociaciones ciudadanas tables
 */

import { supabaseAdmin } from '@/lib/supabase/admin';
import {
  BronzeAsociacionRecord,
  SilverAsociacionRecord,
} from './types';
import { BATCH_SIZE } from '../nomenclator/constants';

/**
 * Batch insert into bronze_ckan_asociaciones_ciudadanas_raw
 */
export async function loadAsociacionesBronze(
  records: BronzeAsociacionRecord[]
  ...
  for (let i = 0; i < records.length; i += BATCH_SIZE) {
    ...
    try {
      const { error } = await supabaseAdmin
        .from('bronze_ckan_asociaciones_ciudadanas_raw')
        .insert(batch);
 ...
 */
function deduplicateBatch(batch: SilverAsociacionRecord[]): SilverAsociacionRecord[] {
  const seen = new Set<string>();
  const deduplicated: SilverAsociacionRecord[] = [];

  for (const record of batch) {
    // Create unique key matching the functional unique index:
    // source_resource_id, asociacion_nombre, municipio_nombre, latitud, longitud, direccion
    ...
    const direccionStr = record.asociacion_direccion || '';
    const municipioStr = record.municipio_nombre || '';
    
    const key = `${record.source_resource_id}|${record.asociacion_nombre}|${municipioStr}|${latitudStr}|${longitudStr}|${direccionStr}`;
...
/**
 * Batch upsert into silver_asociacion_ciudadana
 * Validates INE codes against dim_municipio before inserting
 */
export async function loadAsociacionesSilver(
  records: SilverAsociacionRecord[]
  ...
  const validRecords: SilverAsociacionRecord[] = [];
  ...
  for (let i = 0; i < validRecords.length; i += BATCH_SIZE) {
    ...
    try {
      ...
      const { error } = await supabaseAdmin.rpc('upsert_asociacion_ciudadana_batch', {
...
/**
 * Refresh aggregated facts in fact_asociacion_ciudadana_municipio_agg
 * Uses stored procedure that aggregates directly from silver layer
 */
export async function refreshAsociacionesFacts(): Promise<{
  refreshed: number;
  errors: number;
}> {
  try {
    ...
    const { error: refreshError } = await supabaseAdmin.rpc(
      'refresh_fact_asociacion_ciudadana_municipio_agg'
    );
    ...
    const { count } = await supabaseAdmin
      .from('fact_asociacion_ciudadana_municipio_agg')
      .select('*', { count: 'exact', head: true });

    return { refreshed: count || 0, errors: 0 };
  } catch (error) {
    console.error('Error refreshing asociaciones facts:', error);
