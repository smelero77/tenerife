/**
 * Main orchestrator for CKAN Centros Educativos y Culturales ETL pipeline
 */
...
import { transformCentroEducativoCulturalRecord } from './transform';
import {
  loadCentrosEducativosCulturalesBronze,
  loadCentrosEducativosCulturalesSilver,
  refreshCentrosEducativosCulturalesFacts,
} from './loaders';
import {
  RawCkanCentroEducativoCulturalRecord,
  BronzeCentroEducativoCulturalRecord,
  SilverCentroEducativoCulturalRecord,
} from './types';
import { BATCH_SIZE } from '../nomenclator/constants';

const CKAN_DATASET_ID = '573a49e8-e2fb-4fe4-b47b-ac5dec1bf580';
const CKAN_RESOURCE_ID = 'a06b9a27-2012-40a9-b668-78aa8bd9b6d3';
...
async function createEtlRun(): Promise<string> {
  const { data, error } = await supabaseAdmin
    .from('etl_runs')
    .insert({
      pipeline_name: 'ckan_centros_educativos_culturales',
      status: 'running',
      metadata: {
 ...
 */
export async function runCentrosEducativosCulturales(): Promise<{
  success: boolean;
  runId: string;
  ...
  try {
    ...
    let silverRecords: SilverCentroEducativoCulturalRecord[] = [];
    ...
    try {
      const bronzeRecords: BronzeCentroEducativoCulturalRecord[] = rawRecords.map(
        (record) => ({
      ...
      const bronzeResult = await loadCentrosEducativosCulturalesBronze(bronzeRecords);
    ...
    try {
      ...
      for (const rawRecord of rawRecords) {
        const transformed = transformCentroEducativoCulturalRecord(
          rawRecord as RawCkanCentroEducativoCulturalRecord,
          CKAN_DATASET_ID,
          CKAN_RESOURCE_ID
        );
    ...
    try {
      ...
      const silverResult = await loadCentrosEducativosCulturalesSilver(silverRecords);
    ...
    try {
      const factsResult = await refreshCentrosEducativosCulturalesFacts();
