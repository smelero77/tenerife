/**
 * Database loaders with batch upserts for CKAN centros medicos y farmacias tables
 */

import { supabaseAdmin } from '@/lib/supabase/admin';
import {
  BronzeCentroMedicoFarmaciaRecord,
  SilverCentroMedicoFarmaciaRecord,
} from './types';
import { BATCH_SIZE } from '../nomenclator/constants';

/**
 * Batch insert into bronze_ckan_centros_medicos_farmacias_raw
 */
export async function loadCentrosMedicosFarmaciasBronze(
  records: BronzeCentroMedicoFarmaciaRecord[]
  ...
  for (let i = 0; i < records.length; i += BATCH_SIZE) {
    ...
    try {
      const { error } = await supabaseAdmin
        .from('bronze_ckan_centros_medicos_farmacias_raw')
        .insert(batch);
 ...
 */
function deduplicateBatch(batch: SilverCentroMedicoFarmaciaRecord[]): SilverCentroMedicoFarmaciaRecord[] {
  const seen = new Set<string>();
  const deduplicated: SilverCentroMedicoFarmaciaRecord[] = [];

  for (const record of batch) {
    // Create unique key from constraint fields
    const key = `${record.source_resource_id}|${record.establecimiento_nombre}|${record.municipio_nombre}`;
...
/**
 * Batch upsert into silver_centro_medico_farmacia
 * Validates INE codes against dim_municipio before inserting
 */
export async function loadCentrosMedicosFarmaciasSilver(
  records: SilverCentroMedicoFarmaciaRecord[]
  ...
  const validRecords: SilverCentroMedicoFarmaciaRecord[] = [];
    ...
    try {
      const { error } = await supabaseAdmin
        ...
        .upsert(deduplicatedBatch, {
          onConflict: 'source_resource_id,establecimiento_nombre,municipio_nombre',
...
/**
 * Refresh aggregated facts in fact_centro_medico_farmacia_municipio_agg
 * Uses stored procedure that aggregates directly from silver layer
 */
export async function refreshCentrosMedicosFarmaciasFacts(): Promise<{
  refreshed: number;
  errors: number;
}> {
  try {
    ...
    const { error: refreshError } = await supabaseAdmin.rpc(
      'refresh_fact_centro_medico_farmacia_municipio_agg'
    );
    ...
    const { count } = await supabaseAdmin
      .from('fact_centro_medico_farmacia_municipio_agg')
      .select('*', { count: 'exact', head: true });

    return { refreshed: count || 0, errors: 0 };
  } catch (error) {
    console.error('Error refreshing centros medicos farmacias facts:', error);
