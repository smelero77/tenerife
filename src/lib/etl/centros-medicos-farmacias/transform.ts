...
import { RawCkanCentroMedicoFarmaciaRecord, SilverCentroMedicoFarmaciaRecord } from './types';
...
function normalizePostalCode(value: unknown): string | null {
  ...
  // Handle numeric values (e.g., 38600.0)
...
function normalizePhone(value: unknown): string | null {
  ...
  // Handle numeric values (e.g., 922770539.0)
  const str = typeof value === 'number' 
    ? Math.floor(value).toString() 
    : String(value).trim().split('.')[0]; // Remove decimal part
 ...
 * Format: "servicios profesionales medicos", "fisioterapia rehabilitacion", "centro de salud", etc.
 */
 ...
 */
export function transformCentroMedicoFarmaciaRecord(
  rawRecord: RawCkanCentroMedicoFarmaciaRecord,
  sourceDatasetId: string,
  sourceResourceId: string
): SilverCentroMedicoFarmaciaRecord | null {
  // Required field
  ...
  return {
    ...
    establecimiento_tipo: tipo,
    establecimiento_telefono: telefono,
    establecimiento_email: email,
    establecimiento_web: web,
    establecimiento_direccion: direccion,
    establecimiento_codigo_postal: codigoPostal,
    municipio_nombre: municipioNombre,
    ine_code: ineCode, // Will be validated against dim_municipio in loaders
    latitud,
    longitud,
    establecimiento_actividad: actividad,
  };
}
