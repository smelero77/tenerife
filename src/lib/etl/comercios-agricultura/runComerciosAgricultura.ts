/**
 * Main orchestrator for CKAN Comercios de Agricultura ETL pipeline
 */
...
import { transformComercioAgriculturaRecord } from './transform';
import {
  loadComerciosAgriculturaBronze,
  loadComerciosAgriculturaSilver,
  refreshComerciosAgriculturaFacts,
} from './loaders';
import {
  RawCkanComercioAgriculturaRecord,
  BronzeComercioAgriculturaRecord,
  SilverComercioAgriculturaRecord,
} from './types';
import { BATCH_SIZE } from '../nomenclator/constants';

const CKAN_DATASET_ID = '8c5fae80-fc10-4421-a525-9a15ee0fb56b';
const CKAN_RESOURCE_ID = '0d7f45f8-a80f-4bcb-802f-fe20de1aa8b0';
...
async function createEtlRun(): Promise<string> {
  const { data, error } = await supabaseAdmin
    .from('etl_runs')
    .insert({
      pipeline_name: 'ckan_comercios_agricultura',
      status: 'running',
      metadata: {
 ...
 */
export async function runComerciosAgricultura(): Promise<{
  success: boolean;
  runId: string;
  ...
  try {
    ...
    let silverRecords: SilverComercioAgriculturaRecord[] = [];
    ...
    try {
      const bronzeRecords: BronzeComercioAgriculturaRecord[] = rawRecords.map(
        (record) => ({
      ...
      const bronzeResult = await loadComerciosAgriculturaBronze(bronzeRecords);
    ...
    try {
      ...
      for (const rawRecord of rawRecords) {
        const transformed = transformComercioAgriculturaRecord(
          rawRecord as RawCkanComercioAgriculturaRecord,
          CKAN_DATASET_ID,
          CKAN_RESOURCE_ID
        );
    ...
    try {
      ...
      const silverResult = await loadComerciosAgriculturaSilver(silverRecords);
    ...
    try {
      const factsResult = await refreshComerciosAgriculturaFacts();
