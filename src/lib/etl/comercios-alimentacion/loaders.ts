/**
 * Database loaders with batch upserts for CKAN comercios alimentacion tables
 */

import { supabaseAdmin } from '@/lib/supabase/admin';
import {
  BronzeComercioAlimentacionRecord,
  SilverComercioAlimentacionRecord,
} from './types';
import { BATCH_SIZE } from '../nomenclator/constants';

/**
 * Batch insert into bronze_ckan_comercios_alimentacion_raw
 */
export async function loadComerciosAlimentacionBronze(
  records: BronzeComercioAlimentacionRecord[]
  ...
  for (let i = 0; i < records.length; i += BATCH_SIZE) {
    ...
    try {
      const { error } = await supabaseAdmin
        .from('bronze_ckan_comercios_alimentacion_raw')
        .insert(batch);
 ...
 */
function deduplicateBatch(batch: SilverComercioAlimentacionRecord[]): SilverComercioAlimentacionRecord[] {
  const seen = new Set<string>();
  const deduplicated: SilverComercioAlimentacionRecord[] = [];

  for (const record of batch) {
    // Create unique key matching the functional unique index:
    // source_resource_id, comercio_nombre, municipio_nombre, latitud, longitud, direccion
    ...
    const direccionStr = record.comercio_direccion || '';
    const municipioStr = record.municipio_nombre || '';
    
    const key = `${record.source_resource_id}|${record.comercio_nombre}|${municipioStr}|${latitudStr}|${longitudStr}|${direccionStr}`;
...
/**
 * Batch upsert into silver_comercio_alimentacion
 * Validates INE codes against dim_municipio before inserting
 */
export async function loadComerciosAlimentacionSilver(
  records: SilverComercioAlimentacionRecord[]
  ...
  const validRecords: SilverComercioAlimentacionRecord[] = [];
  ...
  for (let i = 0; i < validRecords.length; i += BATCH_SIZE) {
    ...
    try {
      ...
      const { error } = await supabaseAdmin.rpc('upsert_comercio_alimentacion_batch', {
...
/**
 * Refresh aggregated facts in fact_comercio_alimentacion_municipio_agg
 * Uses stored procedure that aggregates directly from silver layer
 */
export async function refreshComerciosAlimentacionFacts(): Promise<{
  refreshed: number;
  errors: number;
}> {
  try {
    ...
    const { error: refreshError } = await supabaseAdmin.rpc(
      'refresh_fact_comercio_alimentacion_municipio_agg'
    );
    ...
    const { count } = await supabaseAdmin
      .from('fact_comercio_alimentacion_municipio_agg')
      .select('*', { count: 'exact', head: true });

    return { refreshed: count || 0, errors: 0 };
  } catch (error) {
    console.error('Error refreshing comercios alimentacion facts:', error);
