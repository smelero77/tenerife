/**
 * Main orchestrator for CKAN Comercios de Alimentación ETL pipeline
 */

import { supabaseAdmin } from '@/lib/supabase/admin';
import { fetchAllRecords } from '@/lib/ckan/client';
import { transformComercioAlimentacionRecord } from './transform';
import {
  loadComerciosAlimentacionBronze,
  loadComerciosAlimentacionSilver,
  refreshComerciosAlimentacionFacts,
} from './loaders';
import {
  RawCkanComercioAlimentacionRecord,
  BronzeComercioAlimentacionRecord,
  SilverComercioAlimentacionRecord,
} from './types';
import { BATCH_SIZE } from '../nomenclator/constants';

const CKAN_DATASET_ID = '8da30f5b-8233-4da5-b756-482cf310d0c8';
const CKAN_RESOURCE_ID = '8279f63e-269c-4961-b435-7f17ca0ed9cf';
...
async function createEtlRun(): Promise<string> {
  const { data, error } = await supabaseAdmin
    .from('etl_runs')
    .insert({
      pipeline_name: 'ckan_comercios_alimentacion',
      status: 'running',
      metadata: {
 ...
 */
export async function runComerciosAlimentacion(): Promise<{
  success: boolean;
  runId: string;
  ...
  try {
    ...
    let silverRecords: SilverComercioAlimentacionRecord[] = [];
    ...
    try {
      const bronzeRecords: BronzeComercioAlimentacionRecord[] = rawRecords.map(
        (record) => ({
      ...
      const bronzeResult = await loadComerciosAlimentacionBronze(bronzeRecords);
    ...
    try {
      ...
      for (const rawRecord of rawRecords) {
        const transformed = transformComercioAlimentacionRecord(
          rawRecord as RawCkanComercioAlimentacionRecord,
          CKAN_DATASET_ID,
          CKAN_RESOURCE_ID
        );
    ...
    try {
      ...
      const silverResult = await loadComerciosAlimentacionSilver(silverRecords);
    ...
    try {
      const factsResult = await refreshComerciosAlimentacionFacts();
