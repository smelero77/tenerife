/**
 * Main orchestrator for Nationality Distribution 2025 ETL pipeline
 */

import { supabaseAdmin } from '@/lib/supabase/admin';
import { parseNationalityCsvStream } from './parseNationalityCsv';
import { transformNationalityRow } from './transformNationality';
import {
  loadNationalityBronze,
  loadNationalitySilver,
  loadNationalityFactsLocalidad,
  loadNationalityFactsMunicipio,
} from './loaders';
import {
...
import { NATIONALITY_CSV_PATH } from './constants';
import { join } from 'path';

const CSV_FILE_PATH = join(process.cwd(), NATIONALITY_CSV_PATH);
...
async function createEtlRun(): Promise<string> {
  const { data, error } = await supabaseAdmin
    .from('etl_runs')
    .insert({
      pipeline_name: 'nationality_distribution_multi_year_tenerife',
      status: 'running',
        metadata: {
          years: '2000-2025',
          scope: 'Tenerife',
...
export async function runNationalityDistribution2025(): Promise<{
  success: boolean;
  runId: string;
  steps: StepResult[];
  ...
  try {
    ...
    const bronzeRecords: BronzeNationalityRecord[] = [];
    const silverRecords: SilverNationalityRecord[] = [];
    let tenerifeRowCount = 0;

    try {
      console.log(`Reading Nationality CSV from: ${CSV_FILE_PATH}`);
      for await (const { row: rawRow, year } of parseNationalityCsvStream(CSV_FILE_PATH)) {
        summary.totalRowsProcessed++;

        // Transform and filter
        const parsed = transformNationalityRow(rawRow, year, CSV_FILE_PATH);
        if (!parsed) {
          summary.skippedRows++;
          continue;
        }
        ...
        // Build silver record (one per year)
        silverRecords.push({
          ...
          population_spanish: parsed.population_spanish,
          population_foreign: parsed.population_foreign,
        });
      }
    ...
    try {
      const bronzeResult = await loadNationalityBronze(bronzeRecords);
    ...
    try {
      const silverResult = await loadNationalitySilver(silverRecords);
    ...
    // Step 4: Build and load Nationality Facts Localidad (NUC/DIS only)
    const step4Id = await createEtlStep(runId, 'load_nationality_facts_localidad');
    const step4Start = Date.now();
    try {
      const nationalityFactsLocalidad: NationalityFactLocalidadRecord[] = silverRecords
      ...
      const factsResult = await loadNationalityFactsLocalidad(nationalityFactsLocalidad);
      const step4Duration = Date.now() - step4Start;
      await updateEtlStep(step4Id, 'completed');
      steps.push({
        stepName: 'load_nationality_facts_localidad',
        duration: step4Duration,
        success: true,
        stats: factsResult,
      });
      summary.nationalityFactsLocalidadInserted = factsResult.inserted;
    } catch (error) {
      const step4Duration = Date.now() - step4Start;
      const errorMsg =
        error instanceof Error ? error.message : String(error);
    ...
    // Step 5: Build and load Nationality Facts Municipio (Tipo='M' only)
    const step5Id = await createEtlStep(runId, 'load_nationality_facts_municipio');
    const step5Start = Date.now();
    try {
      const nationalityFactsMunicipio: NationalityFactMunicipioRecord[] = silverRecords
        .filter((r) => r.tipo === 'M')
        .map((r) => ({
          ine_code: r.ine_code,
          year: r.year,
      ...
      const factsResult = await loadNationalityFactsMunicipio(nationalityFactsMunicipio);
      const step5Duration = Date.now() - step5Start;
      await updateEtlStep(step5Id, 'completed');
      steps.push({
        stepName: 'load_nationality_facts_municipio',
        duration: step5Duration,
        success: true,
        stats: factsResult,
      });
      summary.nationalityFactsMunicipioInserted = factsResult.inserted;
    } catch (error) {
      const step5Duration = Date.now() - step5Start;
      const errorMsg =
        error instanceof Error ? error.message : String(error);
