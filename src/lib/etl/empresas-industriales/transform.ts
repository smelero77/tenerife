...
import { RawCkanEmpresaIndustrialRecord, SilverEmpresaIndustrialRecord } from './types';
...
function normalizePostalCode(value: unknown): string | null {
  ...
  // Handle numeric values (e.g., 36680.0)
...
function normalizePhone(value: unknown): string | null {
  ...
  // Handle numeric values (e.g., 617066552.0)
  const str = typeof value === 'number' 
    ? Math.floor(value).toString() 
    : String(value).trim().split('.')[0]; // Remove decimal part
 ...
 * Format: "industria automocion taller", "industria alimentacion", "industria construccion", etc.
 */
 ...
 */
export function transformEmpresaIndustrialRecord(
  rawRecord: RawCkanEmpresaIndustrialRecord,
  sourceDatasetId: string,
  sourceResourceId: string
): SilverEmpresaIndustrialRecord | null {
  // Required field
  ...
  return {
    ...
    empresa_codigo_postal: codigoPostal,
