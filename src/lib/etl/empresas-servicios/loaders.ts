/**
 * Database loaders with batch upserts for CKAN empresas servicios tables
 */
...
/**
 * Batch insert into bronze_ckan_empresas_servicios_raw
 */
export async function loadEmpresasServiciosBronze(
  records: BronzeEmpresaServicioRecord[]
  ...
  for (let i = 0; i < records.length; i += BATCH_SIZE) {
    ...
    try {
      const { error } = await supabaseAdmin
        .from('bronze_ckan_empresas_servicios_raw')
        .insert(batch);
 ...
 */
function deduplicateBatch(batch: SilverEmpresaServicioRecord[]): SilverEmpresaServicioRecord[] {
  const seen = new Set<string>();
  const deduplicated: SilverEmpresaServicioRecord[] = [];
  const duplicates: Array<{ key: string; record: SilverEmpresaServicioRecord }> = [];
  ...
  for (const record of batch) {
    ...
    const key = `${record.source_resource_id}|${record.empresa_nombre}|${ineCode}|${lat}|${lon}`;
  ...
  if (duplicates.length > 0) {
    ...
    for (const dup of duplicates) {
      console.log(`  - ${dup.record.empresa_nombre} (${dup.record.municipio_nombre})`);
      console.log(`    INE Code: ${dup.record.ine_code || 'N/A'}`);
      console.log(`    Dirección: ${dup.record.empresa_direccion || 'N/A'}`);
...
/**
 * Batch upsert into silver_empresa_servicio
 * Validates INE codes against dim_municipio before inserting
 */
export async function loadEmpresasServiciosSilver(
  records: SilverEmpresaServicioRecord[]
  ...
  const validRecords: SilverEmpresaServicioRecord[] = [];
  ...
  for (let i = 0; i < validRecords.length; i += BATCH_SIZE) {
    ...
    try {
      ...
      const { error } = await supabaseAdmin.rpc('upsert_empresa_servicio_batch', {
...
/**
 * Refresh aggregated facts in fact_empresa_servicio_municipio_agg
 * Uses stored procedure that aggregates directly from silver layer
 */
export async function refreshEmpresasServiciosFacts(): Promise<{
  refreshed: number;
  errors: number;
}> {
  try {
    ...
    const { error: refreshError } = await supabaseAdmin.rpc(
      'refresh_fact_empresa_servicio_municipio_agg'
    );
    ...
    const { count } = await supabaseAdmin
      .from('fact_empresa_servicio_municipio_agg')
      .select('*', { count: 'exact', head: true });

    return { refreshed: count || 0, errors: 0 };
  } catch (error) {
    console.error('Error refreshing empresas servicios facts:', error);
