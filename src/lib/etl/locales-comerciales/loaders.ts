/**
 * Database loaders with batch upserts for CKAN locales comerciales tables
 */

import { supabaseAdmin } from '@/lib/supabase/admin';
import {
  BronzeLocalComercialRecord,
  SilverLocalComercialRecord,
} from './types';
import { BATCH_SIZE } from '../nomenclator/constants';

/**
 * Batch insert into bronze_ckan_locales_comerciales_raw
 */
export async function loadLocalesComercialesBronze(
  records: BronzeLocalComercialRecord[]
  ...
  for (let i = 0; i < records.length; i += BATCH_SIZE) {
    ...
    try {
      const { error } = await supabaseAdmin
        .from('bronze_ckan_locales_comerciales_raw')
        .insert(batch);
 ...
 */
function deduplicateBatch(batch: SilverLocalComercialRecord[]): SilverLocalComercialRecord[] {
  const seen = new Set<string>();
  const deduplicated: SilverLocalComercialRecord[] = [];

  for (const record of batch) {
    // Create unique key matching the functional unique index:
    // source_resource_id, local_nombre, municipio_nombre, latitud, longitud, direccion
    ...
    const direccionStr = record.local_direccion || '';
    const municipioStr = record.municipio_nombre || '';
    
    const key = `${record.source_resource_id}|${record.local_nombre}|${municipioStr}|${latitudStr}|${longitudStr}|${direccionStr}`;
...
/**
 * Batch upsert into silver_local_comercial
 * Validates INE codes against dim_municipio before inserting
 */
export async function loadLocalesComercialesSilver(
  records: SilverLocalComercialRecord[]
  ...
  const validRecords: SilverLocalComercialRecord[] = [];
  ...
  for (let i = 0; i < validRecords.length; i += BATCH_SIZE) {
    ...
    try {
      ...
      const { error } = await supabaseAdmin.rpc('upsert_local_comercial_batch', {
...
/**
 * Refresh aggregated facts in fact_local_comercial_municipio_agg
 * Uses stored procedure that aggregates directly from silver layer
 */
export async function refreshLocalesComercialesFacts(): Promise<{
  refreshed: number;
  errors: number;
}> {
  try {
    ...
    const { error: refreshError } = await supabaseAdmin.rpc(
      'refresh_fact_local_comercial_municipio_agg'
    );
    ...
    const { count } = await supabaseAdmin
      .from('fact_local_comercial_municipio_agg')
      .select('*', { count: 'exact', head: true });

    return { refreshed: count || 0, errors: 0 };
  } catch (error) {
    console.error('Error refreshing locales comerciales facts:', error);
