/**
 * Main orchestrator for CKAN Locales Comerciales ETL pipeline
 */

import { supabaseAdmin } from '@/lib/supabase/admin';
import { fetchAllRecords } from '@/lib/ckan/client';
import { transformLocalComercialRecord } from './transform';
import {
  loadLocalesComercialesBronze,
  loadLocalesComercialesSilver,
  refreshLocalesComercialesFacts,
} from './loaders';
import {
  RawCkanLocalComercialRecord,
  BronzeLocalComercialRecord,
  SilverLocalComercialRecord,
} from './types';
import { BATCH_SIZE } from '../nomenclator/constants';

const CKAN_DATASET_ID = '2fedd81a-bad0-4ca7-a79d-f9dad6556214';
const CKAN_RESOURCE_ID = '5b2bd6da-10ab-4406-a4b7-fa3972b4fd54';
...
async function createEtlRun(): Promise<string> {
  const { data, error } = await supabaseAdmin
    .from('etl_runs')
    .insert({
      pipeline_name: 'ckan_locales_comerciales',
      status: 'running',
      metadata: {
 ...
 */
export async function runLocalesComerciales(): Promise<{
  success: boolean;
  runId: string;
  ...
  try {
    ...
    let silverRecords: SilverLocalComercialRecord[] = [];
    ...
    try {
      const bronzeRecords: BronzeLocalComercialRecord[] = rawRecords.map(
        (record) => ({
      ...
      const bronzeResult = await loadLocalesComercialesBronze(bronzeRecords);
    ...
    try {
      ...
      for (const rawRecord of rawRecords) {
        const transformed = transformLocalComercialRecord(
          rawRecord as RawCkanLocalComercialRecord,
          CKAN_DATASET_ID,
          CKAN_RESOURCE_ID
        );
    ...
    try {
      ...
      const silverResult = await loadLocalesComercialesSilver(silverRecords);
    ...
    try {
      const factsResult = await refreshLocalesComercialesFacts();
