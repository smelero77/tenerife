...
import { RawCkanLocalComercialRecord, SilverLocalComercialRecord } from './types';
...
function normalizePostalCode(value: unknown): string | null {
  ...
  // Handle numeric values (e.g., 38205.0)
...
function normalizePhone(value: unknown): string | null {
  ...
  // Handle numeric values (e.g., 922277198.0)
  const str = typeof value === 'number' 
    ? Math.floor(value).toString() 
    : String(value).trim().split('.')[0]; // Remove decimal part
 ...
 * Format: "mascotas", "optica ortopedia audicion", "bazar multitienda estanco", etc.
 */
 ...
 */
export function transformLocalComercialRecord(
  rawRecord: RawCkanLocalComercialRecord,
  sourceDatasetId: string,
  sourceResourceId: string
): SilverLocalComercialRecord | null {
  // Required field
